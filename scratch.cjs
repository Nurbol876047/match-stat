...
// 1. Remove videoState from App
content = content.replace(/const \[videoState, setVideoState\] = useState\(\{[\s\S]*?\}\);/, '');

// 2. Define VideoExperimentItem before App
const videoExperimentItemStr = `
...
content = content.replace('export const App = () => {', videoExperimentItemStr + '\nexport const App = () => {');

// 3. Replace the entire video section from `<div className="flex flex-col xl:flex-row gap-12...` down to `<style dangerouslySetInnerHTML...`
const startIdx = content.indexOf('<div className="flex flex-col xl:flex-row gap-12 w-full max-w-7xl items-center xl:items-start">');
if (startIdx === -1) {
...
const endIdx = content.indexOf('{/* Isolated Styles for this specific block */}');
if (endIdx === -1) {
  console.log("Could not find end idx");
  process.exit(1);
}

const replacement = `        <div className="flex flex-col gap-24 w-full items-center xl:items-start max-w-7xl">
          {[
            '/video/demo.mp4',
            '/video/AQNWsZ2J9LLwEtBqmfxPm2qqec4b_0ytC5QTomQkkzWVUeUCyIMZr4vD51mkO5S.mp4',
            '/video/AQMsXKUirsuMRgPOW1qabS74Oi2O1kcRnjx4MD8_Hn800kAba3Mul4dE7C8dIEp.mp4',
            '/video/AQO6pXlDaFbUQ4NL4O3bqsj7dji_uHUH5CYZWHAmWMh6nI1RgEqgcK5QYVTodH4mGRcXbtAuHUh0e.mp4'
          ].map((src, idx) => (
            <VideoExperimentItem key={idx} src={src} />
          ))}
        </div>
